...
class OT2Env(gym.Env):
    ...
    def step(self, action):
        ...
        sim_observation = self.sim.run([action])
        ...
        if current_distance < self.previous_distance:
            ...
            reward -= 0.5  # Penalty for moving further away

        self.previous_distance = current_distance
        reward_for_progress = np.exp(-current_distance)
        reward += reward_for_progress

        # Large bonus reward when close to goal
        if current_distance < 0.001:
            reward += min(50, 50 * (1 - (self.steps / self.max_steps)))  # Adjusted goal achievement reward
            terminated = True
        else:
            terminated = False

        if self.steps >= self.max_steps:
            reward -= 0.01 * self.steps  # Gradual penalty for each additional step taken
            terminated = True
            truncated = True
        else:
            truncated = False
